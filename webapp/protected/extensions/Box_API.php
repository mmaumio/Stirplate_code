<?php 
	...
	class Box_API {
		...
		public function create_folder($name, $parent_id) {
			...
			$params = array('name' => $name, 'parent' => $id);
		...
		public function put_file($filename, $parent_id) {
			...
			$params = array('filename' => "@" . $filename, 'parent_id' => $parent_id, 'access_token' => $this->access_token);
			return json_decode($this->post($url, $params), true);
		}
		...
		public function get_file_download_link($file) {
			$url = $this->build_url("/files/" . $file . "/content");
			return $this->getRedirect($url);
		}
		
		/* Saves the token */
		public function write_token($token, $type = 'file', $path = '') {
		...
		/* Reads the token */
		public function read_token($type = 'file', $json = false, $path = '') {
			if($type == 'file' && file_exists($path . 'token.box')){
				$fp = fopen($path . 'token.box', 'r');
				$content = fread($fp, filesize($path . 'token.box'));
				fclose($fp);
			} else {
				return false;
			}
		...
		/* Loads the token */
		public function load_token($path = '') {
			$array = $this->read_token('file', false, $path);
			if(!$array){
				return false;
			} else {
				if(isset($array['error'])){
					...
					if($this->write_token($token, 'file', $path)){
						$array = json_decode($token, true);
						$this->refresh_token = $array['refresh_token'];
		...
		private static function getRedirect($url) {
			$ch = curl_init();
		...
		private static function post($url, $params = array()) {
			//echo $url . '--URL--<br>';
			//print_r($params);
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
